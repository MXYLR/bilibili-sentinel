...
B站水军评论智能检测与可视化分析系统。基于 Scrapy-Redis 分布式爬虫采集评论/用户/弹幕数据，结合 15 维特征评分引擎 + LLM 多 Provider 语义分析，实现水军账号的自动化识别、评分和报告生成，通过 Flask Dashboard 提供完整的 Web 操作界面。
...
```
    ...
    (Scrapy-Redis)       (Scrapy-Redis)       (Port 5001, 46 routes)
           |                    |                    |
           `-----------+--------'                    |
                       v                             v
...
### 数据采集
- **视频搜索**: 关键词搜索 + 热门排行，采集视频元信息
- **评论采集**: 双排序模式（时间排序耗尽自动切换热度），支持楼中楼主评论，单视频上限 10,000 条
- **用户空间**: 三阶段采集（用户画像 → 投稿列表 → 动态列表），自动关联评论 MID
- **弹幕采集**: XML + Protobuf 双协议，按视频分段拉取

### 水军检测
- **15 维特征评分引擎 (F1-F15)**: 覆盖账号身份、行为模式、内容质量、空间画像四大维度
- **LLM 语义分析**: 多 Provider 支持（DeepSeek V4 / OpenAI GPT-4o / 自定义端点），批量评估评论语义
- **AICU 深度分析**: 对高风险用户回溯历史评论/弹幕/动态，三次融合评分（引擎 50% + LLM 25% + 深度 25%）
...
### Dashboard 控制台
- **系统总览** `/`: 健康卡片 + 统计数据 + 视频列表
- **视频详情** `/video/<bvid>`: 评论展示 + 排行榜 + LLM 标签 + 特征触发图表 + 用户详情弹窗
- **爬虫控制** `/crawler`: 4 爬虫管理 + 种子注入 + 代理池状态 + 登录面板
...
| F12 账号骨架 | 0.10 | 无头像 + ID 乱码 + 无动态 + 无投稿 → 四要素全中 = 100% |
...
│   ├── spiders/
│   │   ├── bilibili_video_spider.py    # 视频搜索爬虫
│   │   ├── bilibili_comment_spider.py  # 评论采集爬虫
│   │   ├── bilibili_user_spider.py     # 用户空间爬虫
│   │   └── bilibili_danmaku_spider.py  # 弹幕采集爬虫
...
│   ├── app.py                  #    Flask 应用 (46 个路由)
│   └── templates/              #    Jinja2 模板 (4 页面)
│       ├── index.html          #    首页 — 视频列表
│       ├── video_detail.html   #    视频详情 — 水军分析结果
│       ├── crawler.html        #    爬虫控制面板
│       └── settings.html       #    系统设置
│
├── config/                     # 配置中心
...
| | `POST /api/crawler/inject` | 注入种子 (BV 号) |
