...
echo.
:: === [2] Start Spiders (minimized, log to file) ===
echo [2/5] Starting Video Spider...
powershell -WindowStyle Minimized -Command "cd '%ROOT%'; & '%VENV_SCRAPY%' crawl bilibili_video > '%ROOT%\data\logs\video.log' 2>&1" >nul 2>&1

echo [3/5] Starting Comment Spider...
powershell -WindowStyle Minimized -Command "cd '%ROOT%'; & '%VENV_SCRAPY%' crawl bilibili_comment > '%ROOT%\data\logs\comment.log' 2>&1" >nul 2>&1

echo   [OK] Both spiders launched (logs: data\logs\)

echo.
:: === [4] Start Dashboard (minimized, log to file) ===
echo [4/5] Starting Dashboard on port 5001...
powershell -WindowStyle Minimized -Command "cd '%ROOT%'; & '%VENV_PYTHON%' dashboard\app.py > '%ROOT%\data\logs\dashboard.log' 2>&1" >nul 2>&1
...
:: Kill hidden powershell/minimized windows that host our processes
echo [3/3] Cleaning up background processes...
