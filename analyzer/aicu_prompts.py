...
- **★ 关键**: 当特征引擎已标记 **账号骨架(F12) ≥ 0.50**（无头像/ID乱码/无动态/无投稿），即使 AICU 无历史数据，也不能降低置信度 — 无历史数据恰恰是批量注册水军号的典型特征，应直接判定为「批量操控型」或「黑产养号型」，置信度 ≥ 70
...
"""
    ...
    if top_features:
        ...
        if f12_val >= 0.50:
            prompt += f"\n⚠️ **账号骨架警告**: F12={f12_val:.2f} ({int(f12_val*4)}/4 项命中)\n"
            prompt += "该用户无头像/ID乱码/无动态/无投稿 — 这是批量注册号的铁证！\n"
            if f12_val >= 0.75:
