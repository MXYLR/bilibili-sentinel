...
必须输出完整推理过程（至少100字），引用具体特征值+评论原文。示例：
```json
{"results":[{"mid":"123456","type_id":6,"type_name":"黑产养号型","confidence":92,"reasoning":"f12骨架=0.80（4/5命中：无头像、bili_默认名、无投稿、默认签名），f3等级=0.65（Lv1低等级），当前视频评论'这次正常了'为模板化灌水内容。综合判定为批量注册养号账号。"}]}
```"""
...
"""
    ...
    prompt += """请分析以上用户，输出 JSON 格式结果。reasoning 字段必须详细（≥100字），引用具体特征值+评论原文。只输出 JSON。"""
    return prompt
