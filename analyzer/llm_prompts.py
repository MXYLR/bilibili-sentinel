...
## 无评论时的判定规则（重要！）
- F12账号骨骼≥0.4（2/5命中）→ 可判type6，即使无评论
...
## 判定
- f12≥0.4 → type6(养号型), conf60-95 (f12=0.4:conf60-70, f12=0.6:conf75-85, f12=1.0:conf90-95)
...
## 输出格式
每个用户输出 JSON。reasoning 80-120字，含特征解读+证据+判定逻辑。

示例1(有评论): "f12=1.0全中(无头像/默认名/无动态/无投稿/默认签名)，F14=100敏感内容。评论'加群领福利QQ群123'含引流，判type4引流广告(conf90)。"
示例2(无评论): "f12=0.4命中2/5(无头像、无动态)，非完整骨骼。但f14=100历史动态含敏感内容，判type8敏感内容(conf85)。无评论时f14本身是强力证据。"
"""
