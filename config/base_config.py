...
DEFAULT_WEIGHTS = {
    # --- 核心身份特征 (权重上调，反映信号强度) ---
    "f1_account_age":         0.07,  # 账号年龄: 注册<30天+评论多 → 新号水军 (原0.08→0.07)
    "f2_follow_ratio":        0.04,  # 粉丝/关注比: 粉丝极少+关注极多 → 引流号
    "f3_level_score":         0.09,  # 用户等级: Lv0-2低等级水军概率高 (原0.10→0.09)
    "f4_avatar_verify":       0.06,  # 头像/认证/签名: 三无账号嫌疑重
    "f5_content_similarity":  0.09,  # 内容相似度: 评论与其他人高度雷同 → 模板化 (原0.10→0.09)
    "f6_time_burst":          0.08,  # 时间爆发: 短时间集中刷评 → 操控迹象
    "f7_sentiment_extreme":   0.04,  # 情感极端: 100%正面或负面 → 非自然表达
    "f8_like_ratio":          0.04,  # 赞评比: 零赞评论 → 无人认同 (原0.05→0.04)
    "f9_registration_batch":  0.04,  # 批量注册: 注册日期高度集中 → 工业号
    "f10_interaction_ring":   0.04,  # 互动小圈子: @提及集中在少数账号
    "f11_vip_anomaly":        0.04,  # 大会员异常: 低等级+VIP → 伪装嫌疑

    # --- 账号空间画像 (v2.6 权重再平衡) ---
    "f12_account_skeleton":   0.10,  # 账号骨架: 无头像+ID乱码+无动态+无投稿 (原0.12→0.10)
    "f13_lottery_repost":     0.07,  # 转发抽奖: 无投稿+全转发抽奖动态 (原0.08→0.07)
    "f14_sensitive_content":  0.10,  # 敏感内容: 历史动态含女拳/以乌/造谣抹黑 (原0.13→0.10)
    "f15_commercial_spam":    0.10,  # v2.8 商业引流: 赌博/色情/加微信/刷单等硬广告 (新增)
}
