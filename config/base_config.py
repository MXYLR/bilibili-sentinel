...
# ============================================================
DEFAULT_WEIGHTS = {
    # --- 核心身份特征 (v2.10 重平衡: 17维, +F16时间规律 +F17自评相似度) ---
    "f1_account_age":         0.07,  # 账号年龄: 注册<30天+评论多 → 新号水军
    "f2_follow_ratio":        0.01,  # 粉丝/关注比: 粉丝极少+关注极多 → 引流号 (0.02→0.01)
    "f3_level_score":         0.09,  # 用户等级: Lv0-2低等级水军概率高
    "f4_avatar_verify":       0.08,  # 头像/认证/签名: 三无账号嫌疑重
    "f5_content_similarity":  0.09,  # 内容相似度: 评论与其他人高度雷同 → 模板化
    "f6_time_burst":          0.08,  # 时间爆发: 短时间集中刷评 → 操控迹象
    "f7_sentiment_extreme":   0.01,  # 情感极端: 100%正面或负面 → 非自然表达 (0.03→0.01)
    "f8_like_ratio":          0.04,  # 赞评比: 零赞评论 → 无人认同
    "f9_registration_batch":  0.01,  # 批量注册: 注册日期高度集中 → 工业号 (0.02→0.01)
    "f10_interaction_ring":   0.01,  # 互动小圈子: @提及集中在少数账号 (0.02→0.01)
    "f11_vip_anomaly":        0.03,  # 大会员异常: 低等级+VIP → 伪装嫌疑 (0.04→0.03)

    # --- 账号空间画像 (v2.9 强化: F12 0.15 作为最强确定性信号) ---
    "f12_account_skeleton":   0.15,  # 账号骨架: 无头像+ID乱码+无动态+无投稿 ★ 最强信号
    "f13_lottery_repost":     0.05,  # 转发抽奖: 无投稿+全转发抽奖动态 (0.07→0.05)
    "f14_sensitive_content":  0.10,  # 敏感内容: 历史动态含女拳/以乌/造谣抹黑
    "f15_commercial_spam":    0.10,  # 商业引流: 赌博/色情/加微信/刷单等硬广告

    # --- v2.10 新增: CleanX 行为模式分析 ---
    "f16_time_regularity":    0.04,  # 时间规律性: 低StdDev=机器人规律发帖 (★来自CleanX)
    "f17_self_similarity":    0.04,  # 自评相似度: 高重复率=模板复制 (★来自CleanX)
}
